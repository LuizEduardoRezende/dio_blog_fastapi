import pytest
import pytest_asyncio
from fastapi import status
from httpx import AsyncClient


@pytest_asyncio.fixture(autouse=True)
async def populate_posts(db):
    from src.schemas.post import PostIn
    from src.services.post import PostService

    service = PostService()
    await service.create(PostIn(title="Post 1", content="Content 1", published=True))
    await service.create(PostIn(title="Post 2", content="Content 2", published=True))
    await service.create(PostIn(title="Post 3", content="Content 3", published=False))


@pytest.mark.parametrize("published,total", [("on", 2), ("off", 1)])
async def test_read_posts_by_status_success(
    client: AsyncClient, access_token: str, published: str, total: int
):
    # Given
    headers = {"Authorization": f"Bearer {access_token}"}
    params = {"published": published, "limit": 10}

    # When
    response = await client.get("/posts/", headers=headers, params=params)

    # Then
    content = response.json()

    assert response.status_code == status.HTTP_200_OK
    assert len(content) == total


async def test_read_posts_limit_sucess(client: AsyncClient, access_token: str):
    # Given
    headers = {"Authorization": f"Bearer {access_token}"}
    params = {"published": "on", "limit": 1}

    # When
    response = await client.get("/posts/", headers=headers, params=params)

    # Then
    content = response.json()

    assert response.status_code == status.HTTP_200_OK
    assert len(content) == 1


async def test_read_posts_not_authenticated_fail(client: AsyncClient):
    # Given
    params = {"published": "on", "limit": 10}

    # When
    response = await client.get("/posts/", params=params)

    # Then
    assert response.status_code == status.HTTP_401_UNAUTHORIZED


async def test_read_posts_empty_paramameters_fail(
    client: AsyncClient, access_token: str
):
    # Given
    headers = {"Authorization": f"Bearer {access_token}"}
    params = {}

    # When
    response = await client.get("/posts/", headers=headers, params=params)

    # Then
    assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY
